...
export async function POST(request: NextRequest) {
  try {
    ...
    // Estimate characters: PDF files typically have ~500-1000 chars per KB
    ...
    // Send PDF directly to GPT-4o using base64 encoding
    // Using the newer responses API format that supports file inputs
    try {
      ...
      // Step 1: Upload the PDF file to OpenAI File API
      ...
      // Note: Standard chat.completions doesn't support file inputs directly
      // We'll use the Assistants API as a workaround
      const assistant = await openai.beta.assistants.create({
        model: 'gpt-4o',
        instructions: `You are an expert document summarizer. Create a concise, well-structured summary of the provided PDF content. Include:
1. A brief overview of the document's main topic and purpose
2. Key points and important information in bullet form
3. Main conclusions, findings, or recommendations
...
Focus on extracting the most valuable and relevant information from the document. Keep the summary comprehensive but not verbose.`,
      ...
      const thread = await openai.beta.threads.create({
        messages: [
          {
            role: 'user',
            content: 'Please provide a comprehensive summary of this PDF document, including key points and main conclusions.',
            attachments: [
              {
                file_id: fileId,
                tools: [{ type: 'file_search' }],
    ...
    console.error('Error in PDF summarizer:', error);
    return NextResponse.json(
